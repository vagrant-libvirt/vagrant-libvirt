      ...
      module NetworkUtil
        ...
        def configured_networks(env, logger)
          ...
          env[:machine].config.vm.networks.each do |type, original_options|
            ...
            options = {
              iface_type:   type,
              netmask:      options[:libvirt__network_address] ?
                              IPAddr.new(options[:libvirt__network_address]).get_mask :
                              '255.255.255.0',
              dhcp_enabled: true,
              forward_mode: 'nat'
            }.merge(options)
            ...
            if options[:type].to_s == 'dhcp' && options[:ip].nil?
              options[:network_name] = options[:libvirt__network_name] ? options[:libvirt__network_name] :
